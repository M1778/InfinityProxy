...
- Stability-scored assignment
  ([ADR-0009](./docs/adr/0009-stability-scored-assignment.md)): windowed probe
  counters with counter folding, a top-working-set handshake loop (256 nodes /
  5 min) so pool coverage never depends on assignment, a freshness decay that
  ages stale verdicts out of Tier A, Tier A/B/cold ordering, feature-flagged (on
